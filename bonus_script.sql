...
select
	...
	teamid
from teams
where yearid = 2016 and w = (select max(w) from teams where yearid = 2016);
...
select
	...
	w
from teams
where yearid = 2016 and w = (select max(w) from teams where yearid = 2016 and teamid = teamid);
...
lateral (select
			...
			w
		 from teams
		 where yearid = 2016 and teams.lgid = leagues.lgid
		 order by w desc
		 limit 1);
...
-- 2.  Another advantage of lateral joins is for when you create calculated columns. In a regular query, when you create a calculated column, you        cannot refer it it when you create other calculated columns. This is particularly useful if you want to reuse a calculated column multiple        times. For example,
	...
	-- results in the error that "total_games" does not exist. 
SELECT
	teamid,
	w,
	l,
...
FROM teams WHERE yearid = 2016 ORDER BY winning_pct DESC;
	...
	-- However, I can restructure this query using the LATERAL keyword:
...
select
	...
	(debut::date - birthdate) / 365 as debut_age,
	(finalgame::date - birthdate) / 365 as retirement_age
from people,
lateral (
...
with wm_games as (
					 select
						...
						yearid
			   		 from allstarfull
					 where startingpos is not null and playerid = 'mayswi01'
			      )
select
	count(distinct a.playerid) as players_with_wm
from allstarfull as a
join wm_games as wm
on a.yearid = wm.yearid and a.gameid = wm.gameid
where a.startingpos is not null and a.playerid <> 'mayswi01';
...
with recursive willie_mays as(
	-- Set "anchor" for recursion with large query
		select
			playerid,
	-- (Setting depth as 0 to be used later (Willie Mays is 0 people from himself)
			0 as depth
		from allstarfull as asf
		where startingpos is not null and playerid = 'mayswi01'
	-- Step that'll be recurred through query
		union
		select
			distinct asf2.playerid,
	-- Adding recursion amount (adding how many places away from Willie Mays)
			wm.depth + 1
		from willie_mays as wm
		join allstarfull as asf
		on wm.playerid = asf.playerid and asf.startingpos is not null
	-- Self join to match columns together, tables aliased for ease of use
		join allstarfull as asf2
		on asf.yearid = asf2.yearid and asf.teamid = asf2.teamid and asf2.startingpos is not null
	-- Depth set to less than 2 so that there's no more than 2 degrees of separation from Willie - termination check for recursion
		where wm.depth < 2
)
	-- End of query used to find amount of players with "2 degrees of separation" from Willie
select
	count(distinct willie_mays.playerid) as played_with_wm
from willie_mays
where depth = 2 and playerid not in (select playerid from willie_mays where depth = 1);
